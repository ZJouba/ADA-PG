...
prep_params = [{
    'preparation_num_imputer_strategy': ['mean', 'median', 'most_frequent', 'constant'],
    'feature_selection_k' : list(range(1, len(feature_importances) + 1))
}]

grid_search_prep = GridSearchCV(fullest_pipeline, prep_params, cv=5, scoring='neg_mean_squared_error', verbose = 1, n_jobs=4)
