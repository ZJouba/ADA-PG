...
def finaldataPreparation(data):
    ...
    encodedLevel = encoder3.fit_transform(temp[['Cabin']])
    temp['Cabin'] = encodedLevel
...
def tuneHyperparams(finalModel, X_train, y_train):
    ...
    """n_estimators = np.arange(100, 500, dtype=int, step=50)
...
def trainModel(finalData):
    ...
    # X_train.drop(['PassengerId', 'Embarked', 'Level'], axis=1)
    ...
    adaFinalModel2 = AdaBoostClassifier(
        base_estimator=finalModel, algorithm='SAMME.R', n_estimators=200, learning_rate=0.5)
...
if os.path.isfile(os.path.join(currentPath, 'allData.p')):
    if input('Model ready data already exists. Redo? [y/n] \t') == 'y':
        ...
        """
        allData = dataPreparation(allData)
